...
The sev-certify project is changing. There will be official support for running the tests without having to boot the SUT. This includes a new test harness called sev-verify. Basically, you can run sev-verify with or without booting the SUT. The challenge is that not all support has been merged yet. Fortunately, there are PRs for any related changes that have not been merged yet.
